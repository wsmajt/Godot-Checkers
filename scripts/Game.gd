...
@onready var MainGeneratePath = GeneratePath.new()
...
func ai_move_piece(from_loc, to_loc) -> void:
...
func move_piece(piece, location)-> void:
	...
	MainGeneratePath.count_pieces(piece_array)
	print(str(MainGeneratePath.evaluate()))
	
func check_winner(piece : Piece, pieces : Array) -> bool:
	...
	if foundEnemyPieces == true:
		for p in foundPieces:
			if p.type in [DataHandler.PieceNames.WHITE_PAWN, DataHandler.PieceNames.BLACK_PAWN]:
				foundPaths.append_array(MainGeneratePath.GeneratePawnsMoveset(p, pieces)) 
			else:
				foundPaths.append_array(MainGeneratePath.GenerateQueensMoveset(p, pieces)) 
		...
		match whosMove:
			DataHandler.Sides.WHITE:
				if piece.type == DataHandler.PieceNames.WHITE_PAWN or piece.type == DataHandler.PieceNames.WHITE_QUEEN:
					legalMoves = MainGeneratePath.get_valid_moves(piece, piece_array)
...
func _play_ai_button_pressed():
	var pieces = MainGeneratePath.get_all_pieces(piece_array, DataHandler.Sides.BLACK)
	for piece in pieces:
		var valid_moves = MainGeneratePath.get_valid_moves(piece, piece_array)
